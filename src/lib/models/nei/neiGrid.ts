...
import type { GridAllocator } from '$lib/types/nei/gridAllocator';
...
export class NeiGrid implements GridAllocator<any> {
	...
	width: number = 1;
	height: number = 0;
	allocator: RowAllocator<GridContents> | null = null;
	...
	BeginAllocation<T extends GridContents>(allocator: RowAllocator<T>): GridAllocator<T> {
		this.FinishRow();
