...
machines['Steam Compressor'] =
	...
	machines['Steam Extractor'] =
	machines['Steam Furnace'] =
	machines['Steam Forge Hammer'] =
	machines['Steam Macerator'] =
		{
			perfectOverclock: 0,
			speed: 0.5,
			power: 0,
...
machines['High Pressure Steam Compressor'] =
	machines['High Pressure Alloy Smelter'] =
	machines['High Pressure Steam Extractor'] =
	machines['High Pressure Steam Furnace'] =
	machines['High Pressure Steam Forge Hammer'] =
	machines['High Pressure Steam Macerator'] =
		{
			perfectOverclock: 0,
			speed: 1,
			power: 0,
...
machines['Steam Squasher'] =
	machines['Steam Separator'] =
	machines['Steam Presser'] =
	machines['Steam Grinder'] =
	machines['Steam Purifier'] =
	machines['Steam Blender'] =
		{
			perfectOverclock: 0,
...
machines['Fluid Shaper'] = {
	...
	power: 0.8,
	parallels: (recipe, choices) =>
		(recipe.voltageTier + 1) * 2 + (recipe.voltageTier + 1) * 3 * choices.widthExpansion,
...
machines['High Current Industrial Arc Furnace'] = {
	...
	power: 1,
	parallels: (recipe, choices) => {
		return IsRecipeType(recipe, 'Plasma Arc Furnace')
			? (recipe.voltageTier + 1) * 8 * choices.w
			: (recipe.voltageTier + 1) * choices.w;
	},
...
machines['Electric Implosion Compressor'] = {
	...
	choices: {
		containmentBlockTier: {
			description: 'Containment Block Tier',
			choices: ['Neutronium', 'Infinity', 'Transcendent Metal', 'SpaceTime', 'Universum']
		}
	}
};
...
machines['Magnetic Flux Exhibitor'] = {
	...
	choices: {
		electromagnet: { description: 'Electromagnet', choices: electroMagnets.map((m) => m.name) }
	}
};
...
machines['Large Fluid Extractor'] = {
	perfectOverclock: 0,
	speed: (recipe, choices) => 1.5 * Math.pow(1.1, choices.coilTier + 1),
	power: (recipe, choices) => 0.8 * Math.pow(0.9, choices.coilTier + 1),
	parallels: (recipe, choices) => (choices.solenoidTier + 2) * 8,
	choices: {
		coilTier: CoilTierChoice,
		solenoidTier: {
			description: 'Solenoid Tier',
			choices: ['MV', 'HV', 'EV', 'IV', 'LuV', 'ZPM', 'UV', 'UHV', 'UEV', 'UIV', 'UMV']
		}
	}
};
...
machines['Industrial Sledgehammer'] = {
	...
	choices: {
		anvilTier: {
			description: 'Anvil Tier',
			choices: ['T1 - Vanilla', 'T2 - Steel', 'T3 - Dark Steel / Thaumium', 'T4 - Void Metal']
		}
	}
};
...
machines['Industrial Precision Lathe'] = {
	perfectOverclock: 0,
	speed: (recipe, choices) =>
		(precisionLatheSpeed[choices.itemPipeCasings] + recipe.voltageTier + 1) / 4,
	power: 0.8,
	parallels: (recipe, choices) =>
		precisionLatheParallels[choices.itemPipeCasings] + (recipe.voltageTier + 1) * 2,
...
machines['Industrial Maceration Stack'] = {
	...
	choices: {
		upgradeChip: { description: 'Upgrade Chip', choices: ['No Upgrade', 'Maceration Upgrade Chip'] }
	}
};
...
machines['Nano Forge'] = {
	...
	choices: {
		tier: {
			description: 'Tier',
			choices: ['T1 (Carbon Nanite)', 'T2 (Neutronium Nanite)', 'T3 (Transcendent Metal Nanite)']
		}
	},
...
machines['Dimensionally Transcendent Plasma Forge'] = {
	...
	parallels: 1,
	choices: {
		convergence: { description: 'Convergence', choices: ['No Convergence', 'Convergence'] }
	},
...
machines['Mega Vacuum Freezer'] = {
	...
	choices: {
		coolant: {
			description: 'Coolant',
			choices: ['No Coolant', 'Molten SpaceTime', 'Spatially Enlarged Fluid', 'Molten Eternity']
		}
	},
...
machines['Industrial Coke Oven'] = {
	...
	choices: {
		casingType: {
			...
			choices: ['Heat Resistant Casings', 'Heat Proof Casings']
		}
	}
};
...
machines['Draconic Evolution Fusion Crafter'] = {
	perfectOverclock: 0,
	speed: (recipe, choices) =>
		1 / Math.max(1, choices.casingTier - (recipe.recipe?.gtRecipe?.voltageTier || 0)),
	power: 1,
	parallels: 1,
