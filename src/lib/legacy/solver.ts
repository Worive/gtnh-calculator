...
function MatchVariablesToConstraints(
	model: Model,
	name: string,
...
function CreateLinkByAlgorithm(
	model: Model,
...
function PreProcessRecipe(recipeModel: RecipeModel, model: Model, collection: LinkCollection) {
	...
	if (gtRecipe && gtRecipe.durationTicks > 0) {
		...
		let perfectOverclocks = Math.min(
			GetParameter(machineInfo.perfectOverclock, recipeModel),
			overclockTiers
		);
...
function CreateAndMatchLinks(group: RecipeGroupModel, model: Model, collection: LinkCollection) {
	...
	for (const key of Object.keys(collection.inputOreDict)) {
		...
		for (const item of oreDict.items) {
			...
			for (const recipe of collection.inputOreDictRecipe[key]) recipe.selectedOreDicts[key] = item;
			if (algorithm === LinkAlgorithm.Ignore) continue;

			CreateLinkByAlgorithm(
				model,
				algorithm,
				group,
				item.id,
...
function ApplySolutionGroup(
	...
	for (const child of group.elements) {
		if (child instanceof RecipeModel) ApplySolutionRecipe(child, solution);
		else if (child instanceof RecipeGroupModel)
